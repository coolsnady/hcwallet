...
// to connect to the hcd process that is launched via Start().
...
// Stop interrupts the running hcd process, and waits until it exits properly.
...
func genCertPair(certFile, keyFile, certFileWallet, keyFileWallet string) error {
	...
	cert, key, err := hcutil.NewTLSCertPair(elliptic.P521(), org, validUntil,
		nil)
	if err != nil {
		return err
	}
