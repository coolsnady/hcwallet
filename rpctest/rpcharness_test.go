...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		// Decode address
		_, err = hxutil.DecodeAddress(addrStr)
		if err != nil {
			t.Fatalf("Unable to decode address %s: %v", addr.String(), err)
		}

		// Try to validate an address that is not owned by wallet
		otherAddress, err := hxutil.DecodeAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
	...
	DevAddr, err := hxutil.DecodeAddress(devSubAddrStr)
	if err != nil {
...
func testGetBalance(r *Harness, t *testing.T) {
	...
	sendAmount := hxutil.Amount(700000000)
...
func testListUnspent(r *Harness, t *testing.T) {
	...
	amountToSend := hxutil.Amount(700000000)
	...
	// Get *hxutil.Tx of send to check the inputs
	...
	for _, txIn := range rawTx.MsgTx().TxIn {
		...
		txInIDs[prevOut.String()] = hxutil.Amount(txIn.ValueIn).ToCoin()
	}
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	amountToSend := hxutil.Amount(1000000)
	...
	fee := hxutil.Amount(totalSpent - totalSent)
...
func testSendMany(r *Harness, t *testing.T) {
	...
	amountsToSend := []hxutil.Amount{700000000, 1400000000}
	...
	addressAmounts := make(map[hxutil.Address]hxutil.Amount)
	totalAmountToSend := hxutil.Amount(0)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	voutAmt := hxutil.Amount(txOut.Value).ToCoin()
	...
	sendAmount := hxutil.Amount(240000000)
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	...
	origTxFee, err := hxutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	// Increase fee by 50%
	newTxFeeCoin := walletInfo.TxFee * 1.5
	newTxFee, err := hxutil.NewAmount(newTxFeeCoin)
	if err != nil {
	...
	newTxFeeActual, err := hxutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	...
	err = wcl.SetTxFee(hxutil.Amount(-1))
	if err == nil {
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	...
	origTicketFee, err := hxutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	newTicketFee, err := hxutil.NewAmount(newTicketFeeCoin)
	if err != nil {
	...
	nominalTicketFee = walletInfo.TicketFee
	newTicketFeeActual, err := hxutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	priceLimit, err := hxutil.NewAmount(2 * mustGetStakeDiffNext(r, t))
	if err != nil {
	...
	err = wcl.SetTicketFee(hxutil.Amount(-1))
	if err == nil {
...
func testPurchaseTickets(r *Harness, t *testing.T) {
	...
	origTicketFee, err := hxutil.NewAmount(walletInfo.TicketFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
	newTicketFee, err := hxutil.NewAmount(walletInfo.TicketFee * 1.5)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
...
func newBlockAt(currentHeight uint32, r *Harness,
	t *testing.T) (uint32, *hxutil.Block, []*chainhash.Hash) {
...
func newBlockAtQuick(currentHeight uint32, r *Harness,
	...
	return block.Header.Height, hxutil.NewBlock(block), blockHashes
}

func getBestBlock(r *Harness, t *testing.T) (uint32, *hxutil.Block, *chainhash.Hash) {
	...
	return curBlockHeight, hxutil.NewBlock(bestBlock), bestBlockHash
}
...
func includesTx(txHash *chainhash.Hash, block *hxutil.Block) bool {
	if len(block.Transactions()) <= 1 {
		return false
	}
...
// includesTx checks if a block contains a transaction hash
func includesStakeTx(txHash *chainhash.Hash, block *hxutil.Block) bool {
...
// spent minus sent.
func getWireMsgTxFee(tx *hxutil.Tx) hxutil.Amount {
	var totalSpent int64
	...
	return hxutil.Amount(totalSpent - totalSent)
}
