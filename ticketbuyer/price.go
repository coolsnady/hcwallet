...
func (t *TicketPurchaser) calcAverageTicketPrice(height int64) (hxutil.Amount, error) {
	...
	// these two prices.
	var avgPricePoolAmt hxutil.Amount
	if t.priceMode == AvgPricePoolMode || t.priceMode == AvgPriceDualMode {
		...
		avgPricePoolAmt = poolValue / hxutil.Amount(poolSize)
