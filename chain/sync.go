...
// notifications from a hxd JSON-RPC server.
...
// the hxd server are performed.  Otherwise, it will listen for notifications
...
func (s *RPCSyncer) Run(ctx context.Context, startupSync bool) error {
	...
	g.Go(func() error {
		...
		if err != nil {
			const op errors.Op = "hxd.jsonrpc.notifyspentandmissedtickets"
		...
		if s.wallet.VotingEnabled() {
			...
			if err != nil {
				const op errors.Op = "hxd.jsonrpc.notifywinningtickets"
				return errors.E(op, err)
			}
...
func (s *RPCSyncer) startupSync(ctx context.Context) error {
	...
	if err != nil {
		const op errors.Op = "hxd.jsonrpc.notifyblocks"
		return errors.E(op, err)
	}
	...
	if err != nil {
		const op errors.Op = "hxd.jsonrpc.rebroadcastwinners"
		return errors.E(op, err)
	}
	_, err = s.rpcClient.RawRequest("rebroadcastmissed", nil)
	if err != nil {
		const op errors.Op = "hxd.jsonrpc.rebroadcastmissed"
		return errors.E(op, err)
	}
