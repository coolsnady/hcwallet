...
func fetchMinedBalance(ns walletdb.ReadBucket) (hxutil.Amount, error) {
	...
	return hxutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt hxutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (hxutil.Amount, error) {
	if len(v) < 9 {
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (hxutil.Amount, bool, error) {
	if len(v) < 9 {
	...
	return hxutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (hxutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read for raw credit amount change "+
			"(expected %d bytes, read %d)", bucketCredits, 9, len(v))
		return 0, false, storeError(apperrors.ErrData, str, nil)
	}
	return hxutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (hxutil.Amount, error) {
	...
	return hxutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (hxutil.Amount, error) {
	...
	return hxutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = hxutil.Amount(byteOrder.Uint64(it.cv))
...
func valueDebit(amount hxutil.Amount, credKey []byte) []byte {
	v := make([]byte, 80)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount hxutil.Amount, block *Block, credKey []byte) error {
...
func extractRawDebitAmount(v []byte) hxutil.Amount {
	return hxutil.Amount(byteOrder.Uint64(v[:8]))
}
...
func valueUnminedCredit(amount hxutil.Amount, change bool, opCode uint8,
...
func fetchRawUnminedCreditAmount(v []byte) (hxutil.Amount, error) {
...
func fetchRawUnminedCreditAmountChange(v []byte) (hxutil.Amount, bool, error) {
	if len(v) < unconfValueSizeLegacy {
		str := "short unmined credit value when look up credit amt change"
		return 0, false, storeError(apperrors.ErrData, str, nil)
	}
	amt := hxutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
...
func valueMultisigOut(sh [ripemd160.Size]byte, m uint8, n uint8,
	...
	blockHeight uint32, amount hxutil.Amount, spentBy chainhash.Hash,
...
func fetchMultisigOut(k, v []byte) (*MultisigOut, error) {
	...
	mso.Amount = hxutil.Amount(byteOrder.Uint64(v[59:67]))
...
func fetchMultisigOutAmount(v []byte) hxutil.Amount {
	return hxutil.Amount(byteOrder.Uint64(v[59:67]))
}
