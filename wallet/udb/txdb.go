...
func fetchMinedBalance(ns walletdb.ReadBucket) (hcutil.Amount, error) {
	...
	return hcutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt hcutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (hcutil.Amount, error) {
	if len(v) < 9 {
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (hcutil.Amount, bool, error) {
	if len(v) < 9 {
	...
	return hcutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (hcutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read for raw credit amount change "+
			"(expected %d bytes, read %d)", bucketCredits, 9, len(v))
		return 0, false, storeError(apperrors.ErrData, str, nil)
	}
	return hcutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (hcutil.Amount, error) {
	...
	return hcutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (hcutil.Amount, error) {
	...
	return hcutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func valueDebit(amount hcutil.Amount, credKey []byte) []byte {
	v := make([]byte, 80)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount hcutil.Amount, block *Block, credKey []byte) error {
...
func extractRawDebitAmount(v []byte) hcutil.Amount {
	return hcutil.Amount(byteOrder.Uint64(v[:8]))
}
...
func fetchRawUnminedCreditAmount(v []byte) (hcutil.Amount, error) {
...
func fetchRawUnminedCreditAmountChange(v []byte) (hcutil.Amount, bool, error) {
	if len(v) < unconfValueSizeLegacy {
		str := "short unmined credit value when look up credit amt change"
		return 0, false, storeError(apperrors.ErrData, str, nil)
	}
	amt := hcutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
...
func valueMultisigOut(sh [ripemd160.Size]byte, m uint8, n uint8,
	...
	blockHeight uint32, amount hcutil.Amount, spentBy chainhash.Hash,
...
func fetchMultisigOut(k, v []byte) (*MultisigOut, error) {
	...
	mso.Amount = hcutil.Amount(byteOrder.Uint64(v[59:67]))
...
func fetchMultisigOutAmount(v []byte) hcutil.Amount {
	return hcutil.Amount(byteOrder.Uint64(v[59:67]))
}
