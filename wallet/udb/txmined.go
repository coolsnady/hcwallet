...
type Store struct {
	...
	acctLookupFunc func(walletdb.ReadBucket, hcutil.Address) (uint32, error)
}
...
func (s *Store) AddCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if invalidated {
		...
		cred := credit{
			...
			block:      block.Block,
			amount:     hcutil.Amount(rec.MsgTx.TxOut[index].Value),
			change:     change,
			spentBy:    indexedIncidence{index: ^uint32(0)},
...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	txOutAmt := hcutil.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) addMultisigOut(ns walletdb.ReadWriteBucket, rec *TxRecord,
	...
	val = valueMultisigOut(p2shScriptHash,
		...
		block.Block.Hash,
		uint32(block.Block.Height),
		hcutil.Amount(rec.MsgTx.TxOut[index].Value),
		*empty,     // Unspent
		0xFFFFFFFF, // Unspent
		rec.Hash)
...
func approvesParent(voteBits uint16) bool {
	return hcutil.IsFlagSet16(voteBits, hcutil.BlockValid)
}
...
func (s *Store) UnspentMultisigCreditsForAddress(ns walletdb.ReadBucket,
	addr hcutil.Address) ([]*MultisigCredit, error) {
...
func (s *Store) unspentMultisigCreditsForAddress(ns walletdb.ReadBucket,
	...
	if sha, ok := addr.(*hcutil.AddressScriptHash); ok {
...
func (s *Store) UnspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket,
	amt hcutil.Amount, height int32, minConf int32, all bool,
...
func (s *Store) balanceFullScan(ns, addrmgrNs walletdb.ReadBucket, minConf int32,
	...
	err := ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		case txscript.OP_SSTX:
			...
			votingAuthorityAmt := hcutil.Amount(0)
			lockedByTicketsAmt := hcutil.Amount(0)
			...
			totalInputAmount := hcutil.Amount(0)
			...
			for _, txin := range rec.MsgTx.TxIn {
				...
				if rawUnmined != nil {
					...
					if int(txin.PreviousOutPoint.Index) < len(tx.TxOut) {
						totalInputAmount += hcutil.Amount(tx.TxOut[txin.PreviousOutPoint.Index].Value)
					}
				} else {
...
type Balances struct {
	Account                 uint32
	ImmatureCoinbaseRewards hcutil.Amount
	ImmatureStakeGeneration hcutil.Amount
	LockedByTickets         hcutil.Amount
	Spendable               hcutil.Amount
	Total                   hcutil.Amount
