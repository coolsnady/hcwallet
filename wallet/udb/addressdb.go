...
func maybeConvertDbError(err error) error {
	...
	return managerError(apperrors.ErrDatabase, err.Error(), err)
}
...
func fetchManagerVersion(ns walletdb.ReadBucket) (uint32, error) {
	...
	if verBytes == nil {
		...
		return 0, managerError(apperrors.ErrDatabase, str, nil)
	}
...
func fetchCryptoKeys(ns walletdb.ReadBucket) ([]byte, []byte, []byte, error) {
	...
	if val == nil {
		...
		return nil, nil, nil, managerError(apperrors.ErrDatabase, str, nil)
	}
...
func fetchWatchingOnly(ns walletdb.ReadBucket) (bool, error) {
	...
	if len(buf) != 1 {
		...
		return false, managerError(apperrors.ErrDatabase, str, nil)
	}
...
func fetchAccountName(ns walletdb.ReadBucket, account uint32) (string, error) {
	...
	if val == nil {
		...
		return "", managerError(apperrors.ErrAccountNotFound, str, nil)
	}
	offset := uint32(0)
...
func fetchAccountByName(ns walletdb.ReadBucket, name string) (uint32, error) {
	...
	if val == nil {
		...
		return 0, managerError(apperrors.ErrAccountNotFound, str, nil)
	}
...
func fetchAccountInfo(ns walletdb.ReadBucket, account uint32, dbVersion uint32) (*dbBIP0044AccountRow, error) {
	...
	if serializedRow == nil {
		str := fmt.Sprintf("account %d not found", account)
		return nil, managerError(apperrors.ErrAccountNotFound, str, nil)
	}
...
func fetchAddressByHash(ns walletdb.ReadBucket, addrHash []byte) (interface{}, error) {
	...
	if serializedRow == nil {
		...
		return nil, managerError(apperrors.ErrAddressNotFound, str, nil)
	}
...
func fetchBlissAddresses(ns walletdb.ReadBucket, account, branch, start, count uint32) ([]hcutil.Address, error) {
...
// database.
func putChainedAddress(ns walletdb.ReadWriteBucket, addr *hcutil.AddressPubKeyHash, account uint32,
...
func fetchAddrAccount(ns walletdb.ReadBucket, addressID []byte) (uint32, error) {
	...
	if val == nil {
		str := "address not found"
		return 0, managerError(apperrors.ErrAddressNotFound, str, nil)
	}
	return binary.LittleEndian.Uint32(val), nil
}
...
func upgradeManager(ns walletdb.ReadWriteBucket) error {
	...
	if version != latestMgrVersion {
		str := fmt.Sprintf("the latest address manager db version %d does "+
			"not equal the expected version %d", version, latestMgrVersion)
...
func PutChainedBlissAddress(ns walletdb.ReadWriteBucket, addr *hcutil.AddressPubKeyHash, account uint32,
	status syncStatus, branch, index uint32) error {
