...
//
// This function MUST be called with the manager lock held for writes.
func (m *Manager) loadAddress(ns walletdb.ReadBucket, address hcutil.Address) (ManagedAddress, error) {
...
func (m *Manager) Address(ns walletdb.ReadBucket, address hcutil.Address) (ManagedAddress, error) {
...
func (m *Manager) AddrAccount(ns walletdb.ReadBucket, address hcutil.Address) (uint32, error) {
	address = normalizeAddress(address)
...
func (m *Manager) ExistsAddress(ns walletdb.ReadBucket, address hcutil.Address) bool {
	address = normalizeAddress(address)
...
func (m *Manager) ImportPrivateKey(ns walletdb.ReadWriteBucket, wif *hcutil.WIF) (ManagedPubKeyAddress, error) {
	...
	pubKeyHash := hcutil.Hash160(serializedPubKey)
...
func (m *Manager) ImportScript(ns walletdb.ReadWriteBucket, script []byte) (ManagedScriptAddress, error) {
	...
	// Prevent duplicates.
	scriptHash := hcutil.Hash160(script)
...
func (m *Manager) MarkUsed(ns walletdb.ReadWriteBucket, address hcutil.Address) error {
...
func (m *Manager) ForEachActiveAddress(ns walletdb.ReadBucket, fn func(addr hcutil.Address) error) error {
	m.mtx.Lock()
	defer m.mtx.Unlock()
...
func (m *Manager) PrivateKey(ns walletdb.ReadBucket, addr hcutil.Address) (key chainec.PrivateKey, done func(), err error) {
...
func (m *Manager) RedeemScript(ns walletdb.ReadBucket, addr hcutil.Address) (script []byte, done func(), err error) {
...
func (m *Manager) LoadBlissAddrs(ns walletdb.ReadBucket, account, branch, start, count uint32) ([]hcutil.Address, error) {
...
func (m *Manager) LoadBlissAddr(ns walletdb.ReadBucket, account, branch, index uint32) (*hcutil.AddressPubKeyHash, error) {
	blissaddr := fetchBlissAddress(ns, account, branch, index)
	blisspubkeyaddrstr := string(blissaddr)
	if blisspubkeyaddrstr == "" {
		return nil, fmt.Errorf("address not found")
	}
	pubkeyhash, err := hcutil.DecodeAddress(blisspubkeyaddrstr)
	if err != nil {
		return nil, err
	}
	addr, ok := pubkeyhash.(*hcutil.AddressPubKeyHash)
