...
func TestStakeInvalidationOfTip(t *testing.T) {
	...
	block1Header := g.generate(hcutil.BlockValid)
	block2Header := g.generate(hcutil.BlockValid)
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		if bal.Total != hcutil.Amount(block2Tx.TxOut[0].Value) {
			t.Errorf("Wrong balance: expected %v got %v", hcutil.Amount(block2Tx.TxOut[0].Value), bal)
		}
		...
		if credits[0].Amount != hcutil.Amount(block2Tx.TxOut[0].Value) {
		...
		*/
		...
		if bal.Total != hcutil.Amount(block1Tx.TxOut[0].Value) {
			t.Errorf("Wrong balance: expected %v got %v", hcutil.Amount(block1Tx.TxOut[0].Value), bal)
		}
		credits, err = s.UnspentOutputs(ns)
		if err != nil {
			return err
		...
		if credits[0].Amount != hcutil.Amount(block1Tx.TxOut[0].Value) {
