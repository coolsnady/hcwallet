...
type ManagedAddress interface {
	...
	// Address returns a hcutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *hcutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the hcutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() hcutil.Address {
	return a.address
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey chainec.PublicKey, compressed bool) (*managedAddress, error) {
	...
	if compressed && pubKey.GetType() == chainec.ECTypeSecp256k1 {
		...
		pubKeyHash = hcutil.Hash160(pubKey.SerializeUncompressed())
	} else {
		pubKeyHash = hcutil.Hash160(pubKey.Serialize())
	}
	address, err := hcutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams,
		pubKey.GetType())
	if err != nil {
		return nil, err
	}
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *hcutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() hcutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := hcutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
