...
var (
	...
	TstRecvTx, _                    = hcutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = hcutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *hcutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, err := hcutil.NewTxFromBytesLegacy(TstRecvSerializedTx)
	if err != nil {
		t.Errorf("failed to deserialize test transaction: %v", err.Error())
		return
	}

	// Switch txout amount to 1 HC.  Transaction store doesn't
	...
	TstDoubleSpendTx := hcutil.NewTx(newDupMsgTx)
	TstDoubleSpendSerializedTx := serializeTx(TstDoubleSpendTx)

	// Create a "signed" (with invalid sigs) tx that spends output 0 of
	// the double spend.
	spendingTx := wire.NewMsgTx()
	spendingTxIn := wire.NewTxIn(wire.NewOutPoint(TstDoubleSpendTx.Sha(), 0, hcutil.TxTreeRegular), []byte{0, 1, 2, 3, 4})
	...
	TstSpendingTx := hcutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: hcutil.Amount(TstRecvTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Sha(),
					Index: 1,
					Tree:  hcutil.TxTreeRegular,
				}: {},
			},
			unmined: map[chainhash.Hash]struct{}{
				*TstRecvTx.Sha(): {},
		...
		},
		{
			...
			bal: hcutil.Amount(TstRecvTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Sha(),
		...
		},
		{
			...
			bal: hcutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: hcutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: hcutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Sha(),
					Index: 0,
		...
		},
		{
			...
			bal: hcutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Sha(),
		...
		},
		{
			...
			unc: hcutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				*wire.NewOutPoint(TstSpendingTx.Sha(), 0, hcutil.TxTreeRegular): {},
				*wire.NewOutPoint(TstSpendingTx.Sha(), 1, hcutil.TxTreeRegular): {},
			},
			unmined: map[chainhash.Hash]struct{}{
		...
		},
		{
			...
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				*wire.NewOutPoint(TstRecvTx.Sha(), 1, hcutil.TxTreeRegular): {},
			},
			unmined: map[chainhash.Hash]struct{}{},
		},
	}
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := hcutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
	...
	op := wire.NewOutPoint(TstSpendingTx.Sha(), 0, hcutil.TxTreeStake)
...
func TestCoinbases(t *testing.T) {
	...
	// Balance should be 0 if the coinbase is immature, 50 HC at and beyond
	// maturity.
	//
