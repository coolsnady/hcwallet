...
var (
	...
	TstRecvTx, _                    = hxutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = hxutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *hxutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, err := hxutil.NewTxFromBytesLegacy(TstRecvSerializedTx)
	if err != nil {
	...
	TstDoubleSpendTx := hxutil.NewTx(newDupMsgTx)
	TstDoubleSpendSerializedTx := serializeTx(TstDoubleSpendTx)

	// Create a "signed" (with invalid sigs) tx that spends output 0 of
	// the double spend.
	spendingTx := wire.NewMsgTx()
	spendingTxIn := wire.NewTxIn(wire.NewOutPoint(TstDoubleSpendTx.Sha(), 0, hxutil.TxTreeRegular), []byte{0, 1, 2, 3, 4})
	...
	TstSpendingTx := hxutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: hxutil.Amount(TstRecvTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Sha(),
					Index: 1,
		...
		},
		{
			...
			bal: hxutil.Amount(TstRecvTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Sha(),
		...
		},
		{
			...
			bal: hxutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: hxutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: hxutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Sha(),
					Index: 0,
		...
		},
		{
			...
			bal: hxutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Sha(),
		...
		},
		{
			...
			unc: hxutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				*wire.NewOutPoint(TstSpendingTx.Sha(), 0, hxutil.TxTreeRegular): {},
				*wire.NewOutPoint(TstSpendingTx.Sha(), 1, hxutil.TxTreeRegular): {},
			},
			unmined: map[chainhash.Hash]struct{}{
		...
		},
		{
			...
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				*wire.NewOutPoint(TstRecvTx.Sha(), 1, hxutil.TxTreeRegular): {},
			},
			unmined: map[chainhash.Hash]struct{}{},
		},
	}
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := hxutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
	...
	op := wire.NewOutPoint(TstSpendingTx.Sha(), 0, hxutil.TxTreeStake)
