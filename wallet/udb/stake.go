...
func (s *StakeStore) insertSStx(ns walletdb.ReadWriteBucket, sstx *hcutil.Tx) error {
...
func (s *StakeStore) dumpSStxHashesForAddress(ns walletdb.ReadBucket, addr hcutil.Address) ([]chainhash.Hash, error) {
	...
	_, addrIsP2SH := addr.(*hcutil.AddressScriptHash)
...
func (s *StakeStore) sstxAddress(ns walletdb.ReadBucket, hash *chainhash.Hash) (hcutil.Address, error) {
	// Access the database and store the result locally.
	...
	if p2sh {
		addr, err = hcutil.NewAddressScriptHashFromHash(thisHash160, s.Params)
	} else {
		addr, err = hcutil.NewAddressPubKeyHash(thisHash160, s.Params, chainec.ECTypeSecp256k1)
	}
	if err != nil {
		str := "failure getting ticket 0th out script hashes from db"
		return nil, stakeStoreError(apperrors.ErrDatabase, str, err)
...
func (s *StakeStore) updateStakePoolUserTickets(ns walletdb.ReadWriteBucket, user hcutil.Address, ticket *PoolTicket) error {
	_, isScriptHash := user.(*hcutil.AddressScriptHash)
	_, isP2PKH := user.(*hcutil.AddressPubKeyHash)
...
func (s *StakeStore) removeStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user hcutil.Address,
...
// database, it adds it.
func (s *StakeStore) updateStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user hcutil.Address, ticket *chainhash.Hash) error {
	_, isScriptHash := user.(*hcutil.AddressScriptHash)
	_, isP2PKH := user.(*hcutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		str := fmt.Sprintf("user %v is invalid", user.EncodeAddress())
...
func stakePoolUserInfo(ns walletdb.ReadBucket, user hcutil.Address) (*StakePoolUser, error) {
	_, isScriptHash := user.(*hcutil.AddressScriptHash)
	_, isP2PKH := user.(*hcutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		str := fmt.Sprintf("user %v is invalid", user.EncodeAddress())
...
func (s *StakeStore) StakePoolUserInfo(ns walletdb.ReadBucket, user hcutil.Address) (*StakePoolUser, error) {
