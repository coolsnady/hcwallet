...
func setup() error {
	...
	return walletdb.Update(db, func(dbtx walletdb.ReadWriteTx) error {
		...
		wif, err := hcutil.NewWIF(privKey, chainParams, chainec.ECTypeSecp256k1)
		if err != nil {
			return err
		}
...
func createUnsignedTicketPurchase(prevOut *wire.OutPoint,
	inputAmount, ticketPrice hcutil.Amount) (*wire.MsgTx, error) {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addr,
		hcutil.Amount(amountsCommitted[0]), ticketFeeLimits)
	if err != nil {
		return nil, err
	}
