...
func p2pkhOutputs(amounts ...hcutil.Amount) []*wire.TxOut {
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	f := func(target hcutil.Amount) (hcutil.Amount, []*wire.TxIn, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			...
			currentTotal += hcutil.Amount(u.Value)
...
func TestNewUnsignedTransaction(t *testing.T) {
	...
	for i, test := range tests {
		...
		if tx.ChangeIndex < 0 {
			...
			changeAmount := hcutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
			if test.ChangeAmount == 0 {
