...
func lookupOutputChain(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails,
	cred udb.CreditRecord) (account uint32, internal bool, address hcutil.Address,
...
func makeTicketSummary(chainClient *hcrpcclient.Client, dbtx walletdb.ReadTx, w *Wallet, details *udb.TicketDetails) *TicketSummary {
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]hcutil.Amount) error {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
...
func flattenBalanceMap(m map[uint32]hcutil.Amount) []AccountBalance {
...
func relevantAccounts(w *Wallet, m map[uint32]hcutil.Amount, txs []TransactionSummary) {
	for _, tx := range txs {
...
func (s *NotificationServer) notifyUnminedTransaction(dbtx walletdb.ReadTx, details *udb.TxDetails) {
	...
	bals := make(map[uint32]hcutil.Amount)
