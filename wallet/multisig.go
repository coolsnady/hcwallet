...
// from them.
func (w *Wallet) MakeSecp256k1MultiSigScript(secp256k1Addrs []hxutil.Address, nRequired int) ([]byte, error) {
	secp256k1PubKeys := make([]*hxutil.AddressSecpPubKey, len(secp256k1Addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*hxutil.AddressScriptHash, error) {
	var p2shAddr *hxutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if apperrors.IsError(err, apperrors.ErrDuplicateAddress) {
				...
				p2shAddr, _ = hxutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*hxutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
...
func (w *Wallet) FetchP2SHMultiSigOutput(outPoint *wire.OutPoint) (*P2SHMultiSigOutput, error) {
	...
	p2shAddr, err := hxutil.NewAddressScriptHashFromHash(
